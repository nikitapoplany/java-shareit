...
public class ItemServiceImpl implements ItemService {
    ...
    public Item createItem(Long userId, Item item) {
        ...
        // Проверка существования запроса, если указан
        if (item.getRequest() != null && item.getRequest().getId() != null) {
            ...
            try {
                ...
                log.warn("Ошибка при проверке запроса: {}", e.getMessage());
                throw e;
            }
        }
    ...
    public Comment addComment(Long userId, Long itemId, Comment comment) {
        log.info("Создание комментария пользователем с ID {} для вещи с ID {}: {}", userId, itemId, comment);
    ...
    /**
     * Создает комментарий к вещи на основе DTO.
     * Этот метод используется контроллером для создания комментария.
     *
    ...
    public CommentDto createComment(Long userId, Long itemId, CommentDto commentDto) {
        log.info("Создание комментария по DTO пользователем с ID {} для вещи с ID {}: {}", userId, itemId, commentDto);
        ...
        Comment savedComment = addComment(userId, itemId, comment);
    ...
    public List<CommentDto> getItemComments(Long itemId) {
        ...
        // Проверка существования вещи
        Item item = getItemById(null, itemId);
        log.debug("Найдена вещь: {}", item);
