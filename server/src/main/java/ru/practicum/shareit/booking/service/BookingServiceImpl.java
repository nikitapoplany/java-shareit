        ...
        try {
            switch (state.toUpperCase()) {
                ...
                case "CURRENT":
                    ...
                    bookings = bookingRepository.findCurrentBookingsByBooker(user, now);
                    break;
                case "PAST":
        ...
        try {
            switch (state.toUpperCase()) {
                ...
                case "CURRENT":
                    ...
                    bookings = bookingRepository.findCurrentBookingsByOwner(owner, now);
                    break;
                case "PAST":
                    log.debug("Поиск прошедших бронирований для вещей владельца с ID {}", userId);
                    bookings = bookingRepository.findPastBookingsByOwner(owner, now);
                    break;
                case "FUTURE":
                    log.debug("Поиск будущих бронирований для вещей владельца с ID {}", userId);
                    bookings = bookingRepository.findFutureBookingsByOwner(owner, now);
                    break;
                case "WAITING":
                    log.debug("Поиск ожидающих бронирований для вещей владельца с ID {}", userId);
                    bookings = bookingRepository.findBookingsByOwnerAndStatus(owner, BookingStatus.WAITING);
                    break;
                case "REJECTED":
                    log.debug("Поиск отклоненных бронирований для вещей владельца с ID {}", userId);
                    bookings = bookingRepository.findBookingsByOwnerAndStatus(owner, BookingStatus.REJECTED);
                    break;
                default:
                    log.warn("Указано неизвестное состояние бронирования: {}", state);
                    throw new ValidationException("Неизвестное состояние: " + state);
